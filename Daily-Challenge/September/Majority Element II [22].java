 ...
 * https://leetcode.com/problems/majority-element-ii/ Thing to note is floor of [n/3] means only 2
 * elements can be possible. Runtime Complexity worstCase : O(n+n)
 */
...
class Solution {
    public List<Integer> majorityElement(int[] nums) {
        ...
        for (int i : frequency.keySet()) {
            if (frequency.get(i) > qualifyingscore)
                result.add(i);
        }
        return result;
    }
